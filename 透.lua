...
local aimRange=50
local stickyHeadshot=true -- 子彈黏頭
local autoFireState=false -- 自動射擊狀態
...
local function shootAtTarget(target)
    if target and target.Character and target.Character:FindFirstChild("Head") then
        ...
        if stickyHeadshot then
            shootAtPosition(headPos)
        else
            shootAtPosition(target.Character.HumanoidRootPart.Position)
        end
    end
end
