...
local toggleRecoilBtn=Instance.new("TextButton")
toggleRecoilBtn.Size=UDim2.new(0,150,0,30)
toggleRecoilBtn.Position=UDim2.new(0,10,0,250) -- 放在原UI下面
toggleRecoilBtn.Text="開啟無後座力"
toggleRecoilBtn.Parent=MainFrame

local noRecoil=true -- 預設啟用

toggleRecoilBtn.MouseButton1Click:Connect(function()
    noRecoil=not noRecoil
    if noRecoil then
        toggleRecoilBtn.Text="關閉無後座力"
        print("無後座力已啟用")
    else
        toggleRecoilBtn.Text="開啟無後座力"
        print("無後座力已關閉")
