...
local function shootAtPosition(pos)
    ...
    local direction=(pos - selfHRP.Position)
    local raycastParams=RaycastParams.new()
    -- 忽略自己角色
    raycastParams.FilterDescendantsInstances={LocalPlayer.Character}
    raycastParams.FilterType=Enum.RaycastFilterType.Blacklist

    local raycastResult=workspace:Raycast(selfHRP.Position, direction, raycastParams)

    if raycastResult and raycastResult.Instance then
        local hitPart=raycastResult.Instance
        local hitCharacter=hitPart.Parent
        if hitCharacter and hitCharacter:FindFirstChild("Humanoid") then
            local enemyHead=hitCharacter:FindFirstChild("Head")
            if enemyHead then
                -- 直接命中敵人頭部
                shootEvent:FireServer(enemyHead.Position)
                return
            end
        end
    end
