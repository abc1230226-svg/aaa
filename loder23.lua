...
local espObjects = {}
local playerStates = {} -- 每個玩家的狀態（autoAim, esp）
local playerButtons = {}
...
local function refreshPlayerList()
    ...
    for _, v in ipairs(getServerPlayers()) do
        ...
        -- 根據狀態設定背景色
        local bgColor
        if s.autoAim and s.esp then
            bgColor = Color3.new(0, 1, 0) -- 綠色
        elseif s.autoAim or s.esp then
            bgColor = Color3.new(1, 1, 0) -- 黃色
        else
            bgColor = Color3.new(0.3, 0.3, 0.3)
        end
